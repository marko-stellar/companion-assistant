...
- **Inspect generated migrations for historical drift before applying them.** When schema changes were added by hand without matching Drizzle snapshots, `drizzle-kit generate` can bundle those already-delivered changes with the current diff. **Why:** replaying that oversized migration risks duplicate-object failures or unrelated production changes. **How to apply:** keep the generated snapshot as the new baseline, but reduce the SQL migration to the intended, idempotent delta after verifying prior migrations already cover the extra statements.
